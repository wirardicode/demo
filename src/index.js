...
async function ocr() {
  ...
  ocrResult.innerHTML =
    '<span class="loading-text">Memproses OCR... █▓▒░</span>';
  translateResult.innerHTML =
    '<span class="loading-text">Waiting for OCR... █▓▒░</span>';

  try {
    const response = await fetch(`${myurl}/imgeOCR`, {
      method: "POST",
      body: formData,
      headers: {
    ...
    ocrResult.innerHTML =
      '<span class="error-text">█ ERROR █ Terjadi kesalahan saat OCR!</span>';
    translateResult.innerHTML =
      '<span class="error-text">█ TRANSLATION CANCELLED █</span>';
    uploadBtn.disabled = false;
    uploadBtn.textContent = "PROSES OCR";
  }
  console.log("OCR function dipanggil...");
}
...
async function translate(text) {
  ...
  try {
    const response = await fetch(`${myurl}/translate`, {
      method: "POST",
      headers: {
    ...
    translateResult.innerHTML = `<span class="success-text">█ TRANSLATION COMPLETE █</span><br><br>Hasil Translate:<br>${translated}`;
  } catch (err) {
    console.error(err);
    translateResult.innerHTML =
